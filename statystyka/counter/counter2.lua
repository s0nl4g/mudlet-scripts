...
function scripts.counter2:add_item(original_text, item)
    local type = item
    ...
    scripts.counter2:add_log(original_text, year, month, day, hour)
    scripts.counter2:add_sum(item, year, month, day, type)
end

function scripts.counter2:add_log(original_text, year, month, day, hour)
    local ret = db:add(scripts.counter2.db_log.counter2_log, {
        year = year,
        month = month,
        day = day,
        text = original_text,
...
function scripts.counter2:add_sum(item, year, month, day, type)
    -- first, update for this type this date
    local retrieved = db:fetch(scripts.counter2.db_daysum.counter2_daysum,
        {
            db:eq(scripts.counter2.db_daysum.counter2_daysum.year, year),
            db:eq(scripts.counter2.db_daysum.counter2_daysum.month, month),
            db:eq(scripts.counter2.db_daysum.counter2_daysum.day, day),
            db:eq(scripts.counter2.db_daysum.counter2_daysum.character, scripts.character_name),
            db:eq(scripts.counter2.db_daysum.counter2_daysum.type, type)
        })


    if not retrieved or table.size(retrieved) == 0 then
        local ret = db:add(scripts.counter2.db_daysum.counter2_daysum, {
            year = year,
            month = month,
            day = day,
            type = type,
    ...
    elseif table.size(retrieved) == 1 then
        ...
        db:update(scripts.counter2.db_daysum.counter2_daysum, update_item)
    ...
    local retrieved = db:fetch(scripts.counter2.db_daysum.counter2_daysum,
        {
            ...
            db:eq(scripts.counter2.db_daysum.counter2_daysum.month, month),
            db:eq(scripts.counter2.db_daysum.counter2_daysum.day, day),
            db:eq(scripts.counter2.db_daysum.counter2_daysum.type, "all")
        })
...
function scripts.counter2:show_short()


    local sql_query = "SELECT * FROM counter2_daysum WHERE character=\"" .. scripts.character_name .. "\" AND type!=\"all\" ORDER BY _row_id ASC"
    local retrieved = db:fetch_sql(scripts.counter2.db_daysum.counter2_daysum, sql_query)
    ...
    for k, v in spairs(count_dict) do
        ...
        local color = scripts.counter.utils:is_rare(name) and "<orange>" or "<LawnGreen>"
...
function scripts.counter2:show_long()
    local sql_query = "SELECT * FROM counter2_daysum WHERE character=\"" .. scripts.character_name .. "\" AND type!=\"all\" ORDER BY _row_id ASC"
    local retrieved = db:fetch_sql(scripts.counter2.db_daysum.counter2_daysum, sql_query)
...
function scripts.counter2:show_logs(year, month, day)


    if( (year == '' or year == nil) and (month == '' or month == nil) and (day == '' or day == nil)) then
        scripts.counter2:show_short()
        return
    end
    ...
    local retrieved = db:fetch_sql(scripts.counter2.db_log.counter2_log, sql_query)
...
function scripts.counter2:reset()


    if not scripts.counter2.retried then
        scripts:print_log("Probujesz wykasowac cala baze zabitych, od tego nie ma odwrotu. Aby wykonac, powtorz komende")
        scripts.counter2.retried = true
    else
        db:delete(scripts.counter2.db_log.counter2_log, db:eq(scripts.counter2.db_log.counter2_log.character, scripts.character_name))
        db:delete(scripts.counter2.db_daysum.counter2_daysum, db:eq(scripts.counter2.db_daysum.counter2_daysum.character, scripts.character_name))
        scripts:print_log("Ok")
    end

    tempTimer(5, function() scripts.counter2.retried = nil end)
end


function alias_licznik_zabici2_short_func()
    scripts.counter2:show_short()
end

function alias_licznik_zabici2_long_func()
    scripts.counter2:show_long()
end

function alias_licznik_zabici2_date_func()
    local sep = string.split(matches[2], "/")
    scripts.counter2:show_logs(sep[1], sep[2], sep[3])
end

function alias_zabici2_reset_func()
    scripts.counter2:reset()
end
